...
L.GeoJSON.VT = L.GridLayer.extend({
    ...
    drawFeature: function (ctx, feature) {
        ...
        if (type === 2 || type === 3) {
            ...
            for (var jl = 0; jl < feature.geometry.length; jl++) {
                var pl = feature.geometry[jl];
                ctx.arc(pl[0] / 16.0, pl[1] / 16.0, 2, 0, Math.PI * 2, true);
            }
        }
    ...
    setStyle: function (ctx, style) {
        ...
        if (fill) {
            ...
            var colorl = this.setOpacity(style.fillColor, style.fillOpacity);
...
String.prototype.colorRgb = function () {
    ...
    var sColorChange = [];
    for (var il = 1; il < 7; il += 2) {
        sColorChange.push(parseInt("0x" + sColor.slice(il, il + 2)));
    }
    return sColorChange;
};